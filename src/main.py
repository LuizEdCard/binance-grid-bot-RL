...
def initialize_components():
    ...
    if alerter is None and config['telegram']['enabled']:
        ...
        alerter = TelegramAlerter(config['telegram']['token'], config['telegram']['chat_id'])
...
def get_status():
    ...
    active_bots_status = {symbol: bot.get_status() for symbol, bot in bots.items() if bot}
    return jsonify({
        'api_status': 'online',
...
def get_market_data():
    ...
    try:
        ...
        formatted_tickers = [{'symbol': t['symbol'], 'price': t['lastPrice'], 'volume': t['volume']} for t in tickers if 'USDT' in t['symbol']]
...
def configure_grid():
    ...
    return jsonify({'message': f'Configuração para {symbol} recebida.', 'received_config': grid_config})
...
def start_grid():
    ...
    if not symbol or not grid_config:
        return jsonify({'error': 'Símbolo e configuração são obrigatórios para iniciar'}), 400

    # Certifique-se de que o frontend envie 'market_type' em grid_config, ou defina padrão
    ...
    thread = threading.Thread(target=run_bot_thread, args=(symbol, grid_config), daemon=True)
...
def stop_grid():
    ...
    if symbol not in bots or not bots[symbol]:
        return jsonify({'error': f'Bot para {symbol} não encontrado ou não está rodando'}), 404
...
def get_grid_status(symbol):
    ...
    if symbol in bots and bots[symbol]:
        ...
        return jsonify({'status': 'inactive', 'message': f'Bot para {symbol} não está ativo.'})
