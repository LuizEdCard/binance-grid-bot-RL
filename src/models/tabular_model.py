...
class TabularTradingModel:
    ...
    def train(self, X: np.ndarray, y: np.ndarray):
        self.model = XGBClassifier(n_estimators=100, max_depth=4, learning_rate=0.1, use_label_encoder=False, eval_metric='logloss')
        self.model.fit(X, y)
        self.save()
...
def prepare_features(df: pd.DataFrame):
    features = df[['close', 'volume', 'rsi', 'macd', 'macd_hist', 'volatility']].fillna(0).values
    return features
