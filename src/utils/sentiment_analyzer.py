...
class SentimentAnalyzer:
    ...
    def __init__(self, model_dir="/home/ubuntu/backend_consolidated/models/sentiment_onnx"):
    ...
    def _load_model(self):
        ...
        try:
            if not os.path.exists(self.model_path):
                log.error(f"Sentiment analysis ONNX model not found at: {self.model_path}")
            ...
            log.info(f"Sentiment analysis model loaded successfully using {self.session.get_providers()}")

        except Exception as e:
            log.error(f"Failed to load sentiment analysis model or tokenizer: {e}", exc_info=True)
            self.tokenizer = None
            self.session = None
    ...
    def analyze(self, text: str) -> dict | None:
        ...
        if not self.session or not self.tokenizer:
            log.error("Sentiment analyzer model or tokenizer not loaded. Cannot analyze.")
            return None
        ...
        try:
            ...
            # Assuming the output logits need decoding (this might need adjustment based on model specifics)
            ...
            # Let's try to parse the decoded output as JSON (often models output JSON strings).
            try:
                ...
                if start_index != -1 and end_index != -1:
                    ...
                    log.debug(f"Sentiment analysis result for '{text[:50]}...': {sentiment_result}")
                    return sentiment_result
                else:
                    log.warning(f"Could not find valid dictionary in model output: {cleaned_output}")
                    return None
            except json.JSONDecodeError as json_err:
                log.error(f"Failed to parse sentiment model output as JSON: {decoded_output}. Error: {json_err}")
                return None
            except Exception as parse_err:
                 log.error(f"Error processing sentiment model output: {decoded_output}. Error: {parse_err}")
                 return None
