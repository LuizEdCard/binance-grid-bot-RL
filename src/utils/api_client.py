...
class APIClient:
    ...
    def _connect(self):
        ...
        if not key or not secret:
            log.error(f\"API_KEY/SECRET not found in .env file. Cannot connect to Binance Production.\")
            self.client = None
            return
        ...
        while retries < MAX_RETRIES:
            try:
                log.info(f\"Attempting to connect to Binance Futures Production (Attempt {retries + 1}/{MAX_RETRIES})...\")
                ...
                log.info(f\"Successfully connected to Binance Futures Production. Server time: {server_time}\")
                return
    ...
    def _make_request(self, method, *args, **kwargs):
        ...
        is_write_operation = method_name in [
            ...
            # Add other write methods if needed (e.g., adjusting leverage, margin type)
        ]

        if self.operation_mode == \"shadow\" and is_write_operation:
            log.info(f\"[SHADOW MODE] Simulating API call: {method_name} args={args} kwargs={kwargs}\")
            ...
            if method_name == \"futures_create_order\":
                ...
                return {
                 ...
                 return {\"code\": \"200\", \"msg\": \"[SHADOW MODE] Simulated cancellation of all orders for symbol.\"}

            return {\"status\": \"simulated_success\"} # Generic simulation for other write ops
        ...
        while retries < MAX_RETRIES:
            try:
                ...
                log.debug(f\"API call successful: {method_name} args={args} kwargs={kwargs}\")
                return response
            except (BinanceAPIException, BinanceRequestException) as e:
                log.warning(f\"API Error on attempt {retries + 1}: {e} (Code: {e.code}, Message: {e.message})\")
                retries += 1
                if retries < MAX_RETRIES:
                    delay = RETRY_DELAY_SECONDS[retries - 1]
                    ...
                    log.error(f\"Max retries reached for API call {method_name}. Error: {e}\")
                    return None
            except ConnectionError as e:
                 log.warning(f\"Connection Error on attempt {retries + 1}: {e}. Attempting reconnect...\")
                 self._connect()
                 if not self.client:
                     log.error(\"Reconnection failed after ConnectionError. Cannot retry API call.\")
                     return None
                 retries += 1
                 if retries >= MAX_RETRIES:
                     log.error(f\"Max retries reached after ConnectionError for API call {method_name}.")
                     return None
            except Exception as e:
                log.error(f\"An unexpected error occurred during API call {method_name}: {e}\")
                return None
    ...
    def place_futures_order(self, symbol, side, order_type, quantity, price=None, timeInForce=None, **kwargs):
        params = {
            \"symbol\": symbol,
            \"side\": side,
            \"type\": order_type,
    ...
    def cancel_futures_order(self, symbol, orderId):
        log.info(f\"Cancelling order ({self.operation_mode.upper()}): symbol={symbol}, orderId={orderId}\")
        return self._make_request(self.client.futures_cancel_order, symbol=symbol, orderId=orderId)
    ...
    def get_futures_order_status(self, symbol, orderId):
        ...
        log.debug(f\"Getting order status ({self.operation_mode.upper()}): symbol={symbol}, orderId={orderId}\")
        if self.operation_mode == \"shadow\":
            log.warning(f\"[SHADOW MODE] get_futures_order_status called for simulated order {orderId}. Returning None. GridLogic should manage state.\")
            return None # Or retrieve from a local cache if implemented
        return self._make_request(self.client.futures_get_order, symbol=symbol, orderId=orderId)
    ...
    def get_open_futures_orders(self, symbol=None):
        ...
        log.debug(f\"Getting open orders ({self.operation_mode.upper()}): symbol={symbol}\")
        if self.operation_mode == \"shadow\":
            log.warning(f\"[SHADOW MODE] get_open_futures_orders called. Returning empty list. GridLogic should manage state.\")
    ...
    def get_futures_klines(self, symbol, interval, startTime=None, endTime=None, limit=500):
        log.debug(f\"Getting klines ({self.operation_mode.upper()}): symbol={symbol}, interval={interval}\")
        return self._make_request(self.client.futures_klines,
                                  symbol=symbol,
                                  interval=interval,
                                  startTime=startTime,
                                  endTime=endTime,
