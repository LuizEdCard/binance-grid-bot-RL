...
class Alerter:
    ...
    def __init__(self, api_client=None):
        ...
        if TELEGRAM_ENABLED:
            if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
                log.error(\"Telegram alerts enabled, but TOKEN or CHAT_ID not found in .env. Disabling Telegram.\")
                self.enabled = False
            else:
                try:
    ...
    def _get_symbol_precision(self, symbol: str, precision_type: str):
        ...
        log.warning(f\"Precision info for {symbol} not found in cache. Using default precision 8.\")
        return 8
    ...
    async def send_message_async(self, text: str, parse_mode=\"MarkdownV2\", photo: bytes = None):
        \"\"\"Asynchronously sends a message (text and optional photo) to the configured Telegram chat.\"\"\"
        if not self.enabled or not self.bot:
            log.debug(f\"Telegram disabled or not initialized. Message not sent: {text[:50]}...\")
            return False
        try:
            if photo:
            ...
            if parse_mode == \"MarkdownV2\":
                ...
                try:
                    if photo:
                         await self.bot.send_photo(chat_id=TELEGRAM_CHAT_ID, photo=photo, caption=text)
                    else:
    ...
    def _generate_chart(self, klines, symbol: str, entry_price=None, tp_price=None, sl_price=None):
        ...
        if not klines or len(klines) < 2:
            ...
            return None
        try:
            # Assuming klines format: [open_time, open, high, low, close, volume, close_time, ...]
            ...
            if entry_price is not None:
                ax.axhline(y=float(entry_price), color=\"blue\", linestyle=\"--\", linewidth=0.8, label=f\"Entry: {entry_price}\")
            if tp_price is not None:
                ax.axhline(y=float(tp_price), color=\"green\", linestyle=\"--\", linewidth=0.8, label=f\"TP: {tp_price}\")
            if sl_price is not None:
                ax.axhline(y=float(sl_price), color=\"red\", linestyle=\"--\", linewidth=0.8, label=f\"SL: {sl_price}\")
    ...
    def send_trade_notification(self, symbol: str, side: str, price: Decimal, qty: Decimal, pnl: Decimal = None, klines_data=None):
        \"\"\"Sends a notification about a filled trade, optionally with a chart.\"\"\"
        side_emoji = \"\U0001F7E2\" if side.upper() == \"BUY\" else \"\U0001F534\" # Green/Red circle
        ...
        if pnl is not None:
            ...
            pnl_emoji = \"\U0001F4B0\" if pnl >= 0 else \"\U0001F4C9\" # Money bag / Chart decreasing
    ...
    def send_trend_recommendation(self, symbol: str, direction: str, entry_price: Decimal, tp_price: Decimal, sl_price: Decimal, confidence: float = None, klines_data=None):
        ...
        arrow = \"\U00002197\" if direction_upper == \"LONG\" else \"\U00002198\" # Arrow up-right / down-right
        price_precision = self.get_price_precision(symbol)

        symbol_safe = telegram.helpers.escape_markdown(symbol, version=2)
        direction_safe = telegram.helpers.escape_markdown(direction_upper, version=2)
        entry_safe = telegram.helpers.escape_markdown(f\"{entry_price:.{price_precision}f}\", version=2)
        tp_safe = telegram.helpers.escape_markdown(f\"{tp_price:.{price_precision}f}\", version=2)
        sl_safe = telegram.helpers.escape_markdown(f\"{sl_price:.{price_precision}f}\", version=2)
        ...
        if klines_data:
            ...
            photo_bytes = self._generate_chart(klines_data, symbol, entry_price, tp_price, sl_price)
