...
class CandlestickPatternAnalyzer:
    ...
    def detect_patterns(self, ohlc_data: pd.DataFrame) -> Dict:
        ...
        """
        if not all(
            col in ohlc_data.columns for col in ["open", "high", "low", "close"]
        ):
            raise ValueError(
    ...
    def analyze_patterns(self, pattern_results: Dict) -> Dict:
        ...
        for pattern_func, result in pattern_results.items():
            ...
            # is_bullish = result['bullish'] # Unused variable removed
            ...
            detected_patterns.append(
                {
                    ...
                    "bullish": (
                        True
                        if latest_signal > 0
                        else (False if latest_signal < 0 else None)
                    ),
        ...
        if high_confidence_bullish > high_confidence_bearish:
            ...
            signal_strength = min(
                high_confidence_bullish / len(self.high_confidence_patterns) * 100, 100
            )
        elif high_confidence_bearish > high_confidence_bullish:
            overall_signal = "bearish"
            signal_strength = min(
                high_confidence_bearish / len(self.high_confidence_patterns) * 100, 100
            )
        # If no high confidence patterns, use the count of all patterns
        elif bullish_count > bearish_count:
            overall_signal = "bullish"
            signal_strength = min(
                (bullish_count - bearish_count) / len(self.patterns) * 100, 100
            )
        elif bearish_count > bullish_count:
            overall_signal = "bearish"
            signal_strength = min(
                (bearish_count - bullish_count) / len(self.patterns) * 100, 100
            )
    ...
    def get_trading_signal(self, ohlc_data: pd.DataFrame) -> Dict:
        ...
        top_patterns = (
            analysis["detected_patterns"][:3] if analysis["detected_patterns"] else []
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Overall Signal: {analysis['overall_signal']} (Strength: {analysis['signal_strength']:.2f}%)"
    )
    print("\nDetected Patterns:")
    for pattern in analysis["detected_patterns"]:
        print(
            f"{pattern['name']}: {'Bullish' if pattern['bullish'] else 'Bearish'} (Strength: {pattern['strength']})"
        )
