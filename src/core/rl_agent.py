...
try:
    ...
    log.warning("TA-Lib library not found for RLAgent. TA-Lib based state features will be unavailable. Please install TA-Lib for full functionality (see talib_installation_guide.md).")
...
class TradingEnvPlaceholder(gym.Env):
    ...
    def __init__(self, config, symbol):
        ...
        self.state_features_config = self.rl_config.get("state_features", { # Now a dict
            "technical_indicators": ["rsi", "atr", "adx"], # Default indicators
            "grid_context": True,
        ...
        state_dim = 0
        # Standard indicators
        if "rsi" in self.state_features_config.get("technical_indicators", []): state_dim += 1
        if "atr" in self.state_features_config.get("technical_indicators", []): state_dim += 1
        if "adx" in self.state_features_config.get("technical_indicators", []): state_dim += 1
        # Grid context
        if self.state_features_config.get("grid_context", False): state_dim += 3
        # Position context
        if self.state_features_config.get("position_context", False): state_dim += 2

        # TA-Lib Indicators
        self.talib_indicator_features = []
        if talib_available:
            for indicator in self.state_features_config.get("talib_indicators", []):
                if indicator == "macd": state_dim += 1 # Using MACD hist
                elif indicator == "bbands": state_dim += 1 # Using %B indicator derived from BBands
                # Add other TA-Lib indicators here
                else: log.warning(f"Unsupported TA-Lib indicator in config: {indicator}")
                self.talib_indicator_features.append(indicator)
        elif self.state_features_config.get("talib_indicators", []):
             log.warning(f"[{self.symbol}] TA-Lib indicators configured but library not available.")
        ...
        if talib_available:
            ...
            if num_patterns > 0:
                 state_dim += num_patterns # One feature per pattern (-100 -> 0, 0 -> 0.5, 100 -> 1)
                 self.talib_pattern_features = self.state_features_config.get("talib_patterns", [])
        elif self.state_features_config.get("talib_patterns", []):
             log.warning(f"[{self.symbol}] TA-Lib patterns configured but library not available.")

        if state_dim == 0:
             log.error(f"[{self.symbol}] Observation space dimension is zero. Check state_features config.")
        ...
        log.info(f"[{self.symbol}] Observation Space: {self.observation_space} (Includes TA-Lib features: {talib_available})")
        log.info(f"[{self.symbol}] TA-Lib Indicators in state: {self.talib_indicator_features}")
        log.info(f"[{self.symbol}] TA-Lib Patterns in state: {self.talib_pattern_features}")
        ...
        log.warning(f"[{self.symbol}] TradingEnvPlaceholder is a placeholder and needs full implementation! Observation/Action spaces are examples.")
    ...
    def step(self, action):
        log.debug(f"[{self.symbol}] TradingEnvPlaceholder step called with action: {action}")
    ...
    def _update_history(self):
        ...
        if len(self.history_df) < self.min_history_needed:
             ...
             fake_df = pd.DataFrame(fake_data, columns=["Open", "High", "Low", "Close", "Volume"])
             ...
             new_candle_df = pd.DataFrame(new_candle_data, columns=["Open", "High", "Low", "Close", "Volume"])
             self.history_df = pd.concat([self.history_df.iloc[1:], new_candle_df], ignore_index=True)
    ...
    def _get_observation(self):
        ...
        if len(self.history_df) < self.min_history_needed:
            log.warning(f"[{self.symbol}] Not enough history ({len(self.history_df)}/{self.min_history_needed}) for observation. Returning zeros.")
        ...
        volume_p = self.history_df["Volume"].values.astype(float)

        # --- Calculate Standard Indicators --- #
        if "rsi" in self.state_features_config.get("technical_indicators", []):
            rsi = 0.5 # Placeholder
            if talib_available:
                try: rsi = talib.RSI(close_p, timeperiod=14)[-1] / 100.0 # Normalize 0-100 to 0-1
                except: pass
            obs_list.append(np.nan_to_num(rsi, nan=0.5)) # Handle potential NaN
        ...
        if "atr" in self.state_features_config.get("technical_indicators", []):
            ...
            if talib_available:
                try:
                    ...
                    if last_close > 0: atr_perc = atr / last_close
                    # Normalize ATR % (e.g., clip at 5% -> 1.0)
                    atr_perc = min(atr_perc / 0.05, 1.0)
                except: pass
            obs_list.append(np.nan_to_num(atr_perc, nan=0.0)) # Handle potential NaN

        if "adx" in self.state_features_config.get("technical_indicators", []):
            adx = 0.25 # Placeholder
            if talib_available:
                try: adx = talib.ADX(high_p, low_p, close_p, timeperiod=14)[-1] / 100.0 # Normalize 0-100 to 0-1
                except: pass
            obs_list.append(np.nan_to_num(adx, nan=0.25)) # Handle potential NaN
        ...
        if talib_available:
            if "macd" in self.talib_indicator_features:
                ...
                try:
                    ...
                    # Simple normalization: scale based on recent price range (approx)
                    price_range = np.ptp(close_p[-20:]) # Peak-to-peak over last 20 periods
                    if price_range > 0:
                         norm_hist = hist[-1] / price_range
                         macd_hist = np.clip((norm_hist + 1.0) / 2.0, 0, 1) # Clip and scale to 0-1
                except: pass
            ...
            if "bbands" in self.talib_indicator_features:
                ...
                try:
                    ...
                    percent_b = np.clip(percent_b, 0, 1) # %B is naturally 0-1 (or slightly outside)
                except: pass
                obs_list.append(np.nan_to_num(percent_b, nan=0.5))
            # Add other TA-Lib indicators here
        else:
            # Add placeholders if TA-Lib indicators were configured but unavailable
        ...
        if talib_available:
            for pattern_name in self.talib_pattern_features:
                ...
                try:
                    ...
                    # log.warning(f"Error calculating pattern {pattern_name}: {e}")
                    pass # Keep placeholder value
                obs_list.append(np.nan_to_num(pattern_val, nan=0.5))
        else:
            # Add placeholders if TA-Lib patterns were configured but unavailable
            num_talib_patterns = len(self.state_features_config.get("talib_patterns", []))
        ...
        if self.state_features_config.get("grid_context", False):
            ...
            norm_levels = 0.5 # Normalize num_levels (e.g., current / max_possible)
            norm_spacing = 0.5 # Normalize spacing (e.g., current / max_possible)
        ...
        if self.state_features_config.get("position_context", False):
            ...
            norm_upnl = 0.5 # Normalize unrealized PNL % (e.g., clip +/- 10% -> 0-1)
        ...
        if observation.shape != self.observation_space.shape:
            log.error(f"[{self.symbol}] Observation shape mismatch! Expected {self.observation_space.shape}, got {observation.shape}. Check feature calculation. Returning zeros.")
            return np.zeros(self.observation_space.shape, dtype=np.float32)
...
class RLAgent:
    ...
    def __init__(self, config: dict, symbol: str):
        ...
        self.rl_config = config.get("rl_agent", {})
        self.symbol = symbol
        self.model_path = os.path.join("/home/ubuntu/grid_trading_backend/models", f"rl_agent_{symbol}")
        self.agent = None
        self.env = None
        self.state_size = self.rl_config.get("state_size", 30)  # Default state size
        self.action_size = self.rl_config.get("action_size", 3)  # Default: hold, buy, sell
    ...
    def _create_env(self, data=None):
        ...
        if data is None:
            ...
            log.warning(f"[{self.symbol}] Using dummy data for environment. Replace with real data in production.")
        ...
        env = TradingEnvironment(data, initial_balance=initial_balance, commission=commission)
    ...
    def setup_agent(self, training=True):
        ...
        if not self.env:
            log.error(f"[{self.symbol}] Failed to create environment. Cannot setup agent.")
            return False
        
        # Get state size from environment
        state_features = self.env._get_state()
        self.state_size = len(state_features)
        log.info(f"[{self.symbol}] State size determined from environment: {self.state_size}")
        ...
        if os.path.exists(model_file) and not training:
            ...
            self.agent = RLTradingAgent(self.state_size, self.action_size, model_path=model_file)
        elif training:
            if os.path.exists(model_file):
                log.info(f"[{self.symbol}] Loading existing model from {model_file} to continue training.")
                self.agent = RLTradingAgent(self.state_size, self.action_size, model_path=model_file)
            else:
            ...
            log.error(f"[{self.symbol}] Model file {model_file} not found and not in training mode.")
            return False
    ...
    def train_agent(self, total_timesteps=None):
        ...
        try:
            ...
            for episode in range(episodes):
                ...
                # Log episode results
                log.info(f"[{self.symbol}] Episode {episode+1}/{episodes}, Reward: {episode_reward:.2f}, Steps: {step}, Epsilon: {self.agent.epsilon:.4f}")
            ...
            log.info(f"[{self.symbol}] Training finished. Total timesteps trained: {self.total_timesteps_trained}")
            return True
            
        except Exception as e:
            log.exception(f"[{self.symbol}] Error during training: {e}")
            return False
    ...
    def predict_action(self, state):
        ...
        try:
            ...
            if len(state) != self.state_size:
                log.error(f"[{self.symbol}] State size mismatch. Expected {self.state_size}, got {len(state)}. Cannot predict.")
                return 0
            ...
            log.error(f"[{self.symbol}] Error predicting action: {e}")
            return 0  # Default action (hold)
    ...
    def save_agent(self):
        ...
        try:
            ...
            log.error(f"[{self.symbol}] Error saving model: {e}")
    ...
    def get_pair_preference_score(self, state):
        ...
        try:
            ...
            log.error(f"[{self.symbol}] Error calculating pair preference: {e}")
...
def rl_training_worker(config_path, symbol):
    ...
    try:
        ...
        log.error(f"[{symbol}] Error loading config {config_path} in training worker: {e}")
        return False
