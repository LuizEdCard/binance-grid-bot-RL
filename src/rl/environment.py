...
class TradingEnvironment(gym.Env):
    ...
    def __init__(self, data: pd.DataFrame, initial_balance: float = 10000.0, commission: float = 0.001, features_window: int = 30, include_sentiment: bool = False, market_types=("spot", "futures")):
        """
        ...
        # Ajuste o número de indicadores conforme adicionado em _calculate_indicators e _get_state
        num_price_features = self.features_window
        num_indicators = 4 # rsi, macd, macd_hist, volatility (ajuste se mudar)
        ...
        self.observation_space_size = num_price_features + num_indicator_features + num_position_features + num_sentiment_features + num_market_features

        # Definir limites para o espaço de observação (aproximados, podem ser refinados)
        # Usar limites amplos (-inf, inf) ou normalizar tudo entre -1 e 1 ou 0 e 1
        self.observation_space = spaces.Box(
            low=-np.inf, high=np.inf, shape=(self.observation_space_size,), dtype=np.float32
        )
        ...
        log.info(f"TradingEnvironment initialized. Observation space size: {self.observation_space_size}, Include Sentiment: {self.include_sentiment}")
    ...
    def _get_state(self, current_sentiment_score: float = 0.0):
        ...
        if self.current_step < self.features_window:
             ...
             log.warning(f"Current step {self.current_step} is less than feature window {self.features_window}. Returning zero state.")
             return np.zeros(self.observation_space.shape, dtype=np.float32)

        frame = self.data.iloc[self.current_step - self.features_window : self.current_step]
        if frame.empty or len(frame) < self.features_window:
            log.warning(f"Insufficient data frame length ({len(frame)}) at step {self.current_step}. Returning zero state.")
            return np.zeros(self.observation_space.shape, dtype=np.float32)
        ...
        for indicator in ["rsi", "macd", "macd_hist", "volatility"]:
            ...
            if len(values) > 0 and not np.isnan(values).all():
                ...
                if max_val > min_val:
                    normalized = (values - min_val) / (max_val - min_val) * 2 - 1 # Normalize to [-1, 1]
                else:
        ...
        features.append(np.clip(self.shares_held / (self.initial_balance / (self.data["close"].mean() if self.data["close"].mean() != 0 else 1)), -1, 1)) # Clip share count relative to potential max shares
        features.append(np.clip((self.balance / self.initial_balance) * 2 - 1, -1, 1)) # Normalize balance to [-1, 1]
        features.append(np.clip((position_value / self.initial_balance) * 2 - 1, -1, 1)) # Normalize position value to [-1, 1]
        ...
        market_one_hot = [1.0 if self.current_market == m else 0.0 for m in self.market_types]
        ...
        if len(final_state) != self.observation_space_size:
             log.error(f"State size mismatch! Expected {self.observation_space_size}, got {len(final_state)}. Padding/truncating.")
             # Pad or truncate (simple approach, might need refinement)
             if len(final_state) > self.observation_space_size:
                 final_state = final_state[:self.observation_space_size]
             else:
                 padding = np.zeros(self.observation_space_size - len(final_state), dtype=np.float32)
    ...
    def step(self, action, current_sentiment_score: float = 0.0):
        ...
        if action_type == 1:  # Comprar
            max_shares = self.balance / (current_price * (1 + commission))
            ...
            if shares_to_buy > 0:
                ...
                if self.balance >= total_cost:
                    ...
                    self.cost_basis = ((self.cost_basis * self.shares_held) + (current_price * shares_to_buy)) / new_total_shares if new_total_shares > 0 else 0
                    self.shares_held = new_total_shares
                    self.total_shares_bought += shares_to_buy
                    self.total_commission_paid += commission_cost
                    self.transaction_history.append({"step": self.current_step, "type": "buy", "shares": shares_to_buy, "price": current_price, "commission": commission_cost, "market": self.current_market})
                    reward = -0.01 # Pequena penalidade por transação
                else:
                    reward = -0.1 # Penalidade maior por falha (saldo insuficiente)
            else:
            ...
            if self.shares_held > 0:
                ...
                if shares_to_sell > 0:
                    ...
                    self.total_commission_paid += commission_cost
                    self.transaction_history.append({"step": self.current_step, "type": "sell", "shares": shares_to_sell, "price": current_price, "commission": commission_cost, "market": self.current_market})
                    ...
                    if self.shares_held == 0: self.cost_basis = 0 # Reset cost basis if position closed
                else:
        ...
        if self.current_step >= len(self.data) - 1:
            ...
            reward += return_pct * 10 # Adicionar recompensa final baseada no retorno total
        ...
        # reward += (info["portfolio_value"] / self.initial_balance - 1) * 0.01 # Pequena recompensa/penalidade contínua
    ...
    def render(self, mode="human"):
        ...
        print(f"Profit/Loss: ${profit_loss:.2f} ({profit_loss/self.initial_balance*100:.2f}%)")
        print("----------------------------")
